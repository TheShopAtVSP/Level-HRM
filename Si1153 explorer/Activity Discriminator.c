...
static double wave1Real[2048];
static double wave1Imaginary[2048];
static double *hart_peak_pos, *hart_peak_amp, *hart_peak_deriv;
static ssize_t hart_peak_cnt;
static int sig1noise = 0;
static double sig1cycles = 10.0;
static double noisewave1[2048];
static double sig1noiseamp = 1.0;
static int seed = 1;
static int sig2wavetype = 0;
static int sig2points = 128;
static double sig2phase = 0.0;
static double wave2[2048];
static int sig2noise = 0;
static double sig2cycles = 10.0;
static double noisewave2[2048];
static double sig2noiseamp = 1.0;
static double waveconv[(2048*2)-1];
static double wavedeconv[2048];
...
{
    int i, j, hbf_sel, hbf_val, showDeconvWarning;
	int threshold_val, auto_threshold;
    double hbf_accm = 0.0, running_avg = 0;
	long shift_filt_ravg = 0;
	...
	GetCtrlVal(mainpnl, MAINPNL_HBFILTER, &hbf_sel);
	if(hbf_sel == 0)
	{
		running_avg = 0;
		for(i=0; i<sig1points; i++)
...
///	SetCtrlVal(mainpnl, MAINPNL_HART_PEAK_COUNT, hart_peak_cnt);
    ...
    DeleteGraphPlot (mainpnl, MAINPNL_CONVGRAPH, -1, VAL_IMMEDIATE_DRAW);
    PlotY (mainpnl, MAINPNL_CONVGRAPH, waveconv,
