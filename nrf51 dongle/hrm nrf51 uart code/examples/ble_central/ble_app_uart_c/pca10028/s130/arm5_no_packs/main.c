...
#define MAX_CONNECTION_INTERVAL MSEC_TO_UNITS(250, UNIT_1_25_MS) /**< Determines maximum connection interval in millisecond. */
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_ADV_REPORT:
        {
			...
			if (found_name)
			{
				if (strlen(m_target_periph_name1) != 0)
				{
					//if(memcmp(m_target_periph_name, dev_name.p_data, dev_name.size) == 0)
					if(memcmp(m_target_periph_name1, dev_name.p_data, 9) == 0)
					{
						printf("Connecting to %s\n", m_target_periph_name1);
						do_connect = true;
					}
					if(memcmp(m_target_periph_name2, dev_name.p_data, 9) == 0)
					{
						printf("Connecting to %s\n", m_target_periph_name2);
						do_connect = true;
					}
					if(memcmp(m_target_periph_name3, dev_name.p_data, 9) == 0)
					{
						printf("Connecting to %s\n", m_target_periph_name3);
						do_connect = true;
					}
					if(do_connect == false);
						printf(" %s is the Wrong Name.", dev_name.p_data);
				}
			}
