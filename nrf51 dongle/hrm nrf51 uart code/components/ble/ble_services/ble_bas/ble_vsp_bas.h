...
//typedef struct
//{
//    ble_bas_evt_type_t           ble_bas_evt_type;                      /**< Type of the event.*/
////    union
////    {
////        ble_bas_pkt_write_t      ble_bas_pkt_write;                     /**< The DFU packet received. This field is when the @ref ble_dfu_evt_type field is set to @ref BLE_DFU_PACKET_WRITE.*/
////        ble_pkt_rcpt_req_t pkt_rcpt_notif_req;                    /**< Packet receipt notification request. This field is when the @ref ble_dfu_evt_type field is set to @ref BLE_DFU_PKT_RCPT_NOTIF_ENABLED.*/
////    } evt;
//} ble_bas_evt_t;
...
/**@brief VSP DFU Control Service event handler type. */
typedef void (*ble_bas_data_handler_t) (ble_bas_t * p_nus, uint8_t * p_data, uint16_t length);
...
typedef struct
{
	...
	uint8_t                       initial_batt_state;             /**< Initial battery level */	
	ble_srv_cccd_security_mode_t  battery_level_char_attr_md;     /**< Initial security level for battery characteristics attribute */
	ble_srv_cccd_security_mode_t  battery_state_char_attr_md;     /**< Initial security level for battery characteristics attribute */	
    ble_gap_conn_sec_mode_t       battery_level_report_read_perm; /**< Initial security level for battery report read attribute */
	ble_gap_conn_sec_mode_t       battery_state_report_read_perm; /**< Initial security level for battery report read attribute */	
	ble_bas_data_handler_t   	  dfu_control_data_handler;       /**< Event handler to be called for handling dfu control data. */	
} ble_bas_init_t;
...
struct ble_bas_s
{
	...
	ble_gatts_char_handles_t      bas_pkt_handles;                /**< Handles related to the DFU Packet characteristic. */	
    ble_gatts_char_handles_t      battery_level_handles;          /**< Handles related to the Battery Level characteristic. */
	ble_gatts_char_handles_t      battery_state_handles;          /**< Handles related to the Battery Level characteristic. */	
	ble_gatts_char_handles_t 	  dfu_control_handle;
    uint16_t                      report_ref_handle;              /**< Handle of the Report Reference descriptor. */
    uint8_t                       battery_level_last;             /**< Last Battery Level measurement passed to the Battery Service. */
	uint8_t                       battery_state_last;             /**< Last Battery Level measurement passed to the Battery Service. */	
...
uint32_t ble_bas_battery_state_update(ble_bas_t * p_bas, uint8_t battery_level);
