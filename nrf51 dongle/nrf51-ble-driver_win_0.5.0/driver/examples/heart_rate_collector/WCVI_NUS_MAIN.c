...
static void on_descriptor_discover_response(const ble_gattc_evt_t * const p_ble_gattc_evt)
{
    ...
    //printf("Press enter to toggle notifications on_descriptor_discover_response\n"); fflush(stdout);
}
...
static uint32_t service_discovery_start()
{
	...
	{
		//printf("Discovering primary services finished\n"); fflush(stdout);
		services_discovered_flag = 1;
	}
    return NRF_SUCCESS;
}
...
int main(int argc, char *argv[])
{
	...
	while(ble_state < 44)
	{
		...
		case 5:  // wait for char discovery 
			if(char_discovery_completed_flag || (service_index != 1))
			{
		...
		case 7:  // wait for char discovery 
			if(prim_srvc_disc_rsp_flag)
			{
				prim_srvc_disc_rsp_flag = 0; 
				ble_state = 8;
		...
		case 8:  // wait for start char_discovery_start
			service_index = 2;
			service_discovery_start();
				ble_state = 9;
		break;
		
		case 9:  // wait for char discovery 
			if(prim_srvc_disc_rsp_flag)
			{
				prim_srvc_disc_rsp_flag = 0;
				ble_state = 10;
		...
		case 10:  // wait for start char_discovery_start
			service_index = 3;
			service_discovery_start();
				ble_state = 11;
		break;
		
		case 11:  // wait for char discovery 
			if(prim_srvc_disc_rsp_flag)
			{
				prim_srvc_disc_rsp_flag = 0;
				ble_state = 12;
		...
		case 12:  // wait for start char_discovery_start
			service_index = 4;
			service_discovery_start();
				ble_state = 13;
		break;
		
		case 13:  // wait for char discovery 
			if(prim_srvc_disc_rsp_flag)
			{
				prim_srvc_disc_rsp_flag = 0;
				ble_state = 13;
...
/*
		case 8:  // start descriptor discovery 
				descr_discovery_start();
				ble_state = 9;
		break;
		
		case 9:  // wait for descriptor discovery 
			if(descr_discovered_flag)
				ble_state = 10;
		break;
...
/*
	...
	while(!connected_bonded_and_ready) {Delay(0.25);}
	// Scan, connect, bond, exchange security params, charactoristics discovered. 
	error_code = service_discovery_start();
	
	//printf("descr_discovery_start()\n"); fflush(stdout);
	//descr_discovery_start();
	ProcessSystemEvents();
	
    while (!descr_discovery_completed_flag){Delay(0.25);} 
    {
		printf("descr_discovery_start() Completed\n"); fflush(stdout);	   	
