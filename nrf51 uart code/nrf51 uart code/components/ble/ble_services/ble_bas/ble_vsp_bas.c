...
#include "ble_vsp_srv_common.h"
#include "app_util.h"


#define INVALID_BATTERY_LEVEL 255
#define INVALID_BATTERY_STATE 255
...
//	if ( (p_evt_write->handle == p_bas->dfu_control_handle.value_handle) && (p_bas->dfu_control_data_handler != NULL) )
//    {
//        p_bas->dfu_control_data_handler(p_bas, p_evt_write->data, p_evt_write->len);
//    }
	if ( (p_evt_write->len == 1) && (p_bas->dfu_control_data_handler != NULL) )
    {
        p_bas->dfu_control_data_handler(p_bas, p_evt_write->data, p_evt_write->len);
    }
	if ( (p_evt_write->handle == p_bas->battery_level_handles.cccd_handle) && (p_evt_write->len == 2) )
	{
	...
	if ((p_evt_write->handle == p_bas->battery_state_handles.cccd_handle) && (p_evt_write->len == 2) )
	{
		// CCCD written, call application event handler
		if (p_bas->evt_handler != NULL)
		{
...
/**@brief Function for adding the Battery States characteristic.
 *
 * @param[in]   p_bas        Battery Service structure.
 * @param[in]   p_bas_init   Information needed to initialize the service.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t battery_state_char_add(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    uint32_t            err_code;
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    ble_uuid_t          ble_uuid;
    ble_gatts_attr_md_t attr_md;
    uint8_t             initial_battery_state;
    uint8_t             encoded_report_ref[BLE_SRV_ENCODED_REPORT_REF_LEN];
    uint8_t             init_len;
	
    // Add Battery State characteristic
	memset(&cccd_md, 0, sizeof(cccd_md));

	BLE_GAP_CONN_SEC_MODE_SET_OPEN(&cccd_md.read_perm);
	cccd_md.write_perm = p_bas_init->battery_state_char_attr_md.cccd_write_perm;
	cccd_md.vloc       = BLE_GATTS_VLOC_STACK;
    ...
    BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_UUID_BATTERY_STATE_CHAR);

    memset(&attr_md, 0, sizeof(attr_md));

    attr_md.read_perm  = p_bas_init->battery_state_char_attr_md.read_perm;
    attr_md.write_perm = p_bas_init->battery_state_char_attr_md.write_perm;
    attr_md.vloc       = BLE_GATTS_VLOC_STACK;
    attr_md.rd_auth    = 0;
    attr_md.wr_auth    = 0;
    attr_md.vlen       = 0;

    initial_battery_state = p_bas_init->initial_batt_state;
    ...
    attr_char_value.init_offs = 0;
    attr_char_value.max_len   = sizeof(uint8_t);
    attr_char_value.p_value   = &initial_battery_state;

    err_code = sd_ble_gatts_characteristic_add(p_bas->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_bas->battery_state_handles);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    ...
    if (p_bas_init->p_report_ref != NULL)
    {
        ...
        attr_md.read_perm = p_bas_init->battery_state_report_read_perm;
        BLE_GAP_CONN_SEC_MODE_SET_NO_ACCESS(&attr_md.write_perm);
        ...
        err_code = sd_ble_gatts_descriptor_add(p_bas->battery_state_handles.value_handle,
                                               &attr_char_value,
                                               &p_bas->report_ref_handle);
        if (err_code != NRF_SUCCESS)
        {
...
uint32_t ble_bas_init(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    ...
    p_bas->battery_state_last        = INVALID_BATTERY_STATE;
    p_bas->dfu_control_data_handler  = p_bas_init->dfu_control_data_handler;	
	...
	// Add battery state characteristic
	err_code = battery_state_char_add(p_bas, p_bas_init);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }				
...
uint32_t ble_bas_battery_state_update(ble_bas_t * p_bas, uint8_t battery_state)
{
    uint32_t err_code = NRF_SUCCESS;
    ble_gatts_value_t gatts_value;

    if (battery_state != p_bas->battery_state_last)
    {
        ...
        gatts_value.p_value = &battery_state;

        // Save new battery value.
        p_bas->battery_state_last = battery_state;

        // Update database.
        err_code = sd_ble_gatts_value_set(p_bas->conn_handle,
                                          p_bas->battery_state_handles.value_handle,
                                          &gatts_value);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
        ...
        if (p_bas->conn_handle != BLE_CONN_HANDLE_INVALID)
        {
            ...
            hvx_params.handle = p_bas->battery_state_handles.value_handle;
