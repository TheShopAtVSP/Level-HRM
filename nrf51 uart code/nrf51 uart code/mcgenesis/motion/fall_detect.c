	...
	switch ( fall.state )
	{
		...
		case POTENTIAL_FALL:		
			...
			if( vel_mm_s <= -3000 ) {
				...
				if (imu_debug) app_trace_log("Possible Fall!!!\r");
		...
		case WAIT_IMPACT:
			...
			if( imu_debug ) app_trace_log("Mag: %04u, Impact: %04u\r",(unsigned int)accel_buf.fifo[a_tail].m, (int)fall.impact);
			break;
		...
		case SIT_DOWN:
			if( false ) {
				...
				if (imu_debug) app_trace_log("Sit Down Detected\r");
				fall.state = NO_FALL;
			}
			break;
		...
		case STAND_UP:
			if( false ) {
				...
				//save Sitting activity
				if (imu_debug) app_trace_log("Stand Up Detected\r");
				fall.state = NO_FALL;
			}
			break;
		...
		case STAIR_DOWN:
			//track stair count
			if (imu_debug) app_trace_log("Step Down Detected\r");
			fall.state = NO_FALL;
			break;
		
		case STAIR_UP:
			//track stair count
			if (imu_debug) app_trace_log("Step Up Detected\r");
			fall.state = NO_FALL;
			break;
		...
		case FALL:
			...
			if( fall.state == NO_FALL ) {
				if (imu_debug) app_trace_log("Not a Fall!!!\r");
			}
			break;
		
		case CRITICAL_FALL:
			//Fall is for real...
			if (imu_debug) app_trace_log("Fall Detected!!!\r");
			fall.state = NO_FALL;
			break;
