...
#define MIN_DETECTION_CNT   10					//Don't Start accumulating Steps unless we see at least X in a row
...
/// \return .
///
void initStepDetect( bool debug )
{	
	uint32_t i;
...
/// \return .
///
void setStepCnt( uint32_t step_cnt )
{
...
TSTEP_FEEDBACK stepStateMachine( uint32_t time, uint32_t mag )
{
        ...
        switch (stepDetectSV) {
            ...
            case FIND_SLOPE:
				if( time < Step.Minima.time || time < Step.Maxima.time ) {
					...
					if (mot_debug) {
						app_trace_log("Step_Algo: Timers Invalid!\r");	
					}
            ...
            case MAXIMA_HUNT: //Waiting for a Possible Maxima Inflection
                ...
                {   //Inflection found!
                    ...
                    {   //Only consider a Potential Maxima that is greater than 1g threshold...																			
						...
						{
							//if (mot_debug) app_trace_log("Poss Max: %04u\r", (unsigned int)Step.possMaxima.mag);
            ...
            case VERIFY_MAXIMA:				
                ...
                {   //MAXIMA has taken too long to fall below DynamicDeltaMag, null out Potential
					...
					//if (imu_debug) app_trace_log("Step_Algo: Max TO\r");	
                    Step.BlockAccum = true;     //Have to see X Valid Steps before counting again
                    Step.uDynamicDeltaMag = Step.uMinDeltaMag;
                ...
                else if (mag < (uint32_t)iDynamicThres)
                {
                    ...
                    if( stepPeriod > 2*MAX_TIME_MAX2MAX ) 
					{
						Step.BlockAccum = true;
						Step.Startup_Count = 0;
						//if (mot_debug) app_trace_log("Step Clear\r");
						res = STEP_FILTER_CLR;
                    }
					...
					{	//Don't allow Peaks to add to Step Count if a low variance Cadence is not established...
						...
						//if (mot_debug) app_trace_log("Step Start\r");
					...
					{	//Looks like an actual Step Maxima!
						...
						{
							Step.BlockAccum = true;
							//if (mot_debug) app_trace_log("High Variance Motion\r");
						}
						...
						else
						{
							...
							{   //Amplitude is not high enough to match the frequency of a person fast walking...
								...
								Step.BlockAccum = true;
								Step.Startup_Count = 0;
								//if (mot_debug) app_trace_log("Non Stepping Motion\r");
								res = STEP_FILTER_CLR;
							}
							...
							{   //Frequency too fast for bipedal motion
								Step.BlockAccum = true;
								Step.Startup_Count = 0;
								//if (mot_debug) app_trace_log("Motion Too Fast\r");
								res = STEP_FILTER_CLR;
							}
							else
							{	//all the conditions are met, count away!
								...
								Step.Abs_Count++;
								if (mot_debug) app_trace_log("step: %01u\r", Step.Abs_Count);
