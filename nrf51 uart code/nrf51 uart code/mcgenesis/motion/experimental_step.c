...
static struct {
	...
	struct {
		uint32_t ptr;
		uint32_t periods[SAMP_SIZE];	//time between step peaks in milliseconds
		uint32_t amplitudes[SAMP_SIZE];	//highest value of Peak in g-scale
...
/// \return .
///
void x_initStepDetect( bool debug )
{	
	uint32_t i;
...
/// \return .
///
void x_setStepCnt( uint32_t step )
{
	Step.Rel_Count = step;
}
...
TSTEP_FEEDBACK x_stepStateMachine( uint32_t time, int32_t mag )
{
        ...
        switch (stepDetectSV) {
            ...
            case FIND_SLOPE:
				if( time < Step.Minima.time || time < Step.Maxima.time ) {
					...
					if (mot_debug) {
						app_trace_log("exp_step: Timers Invalid!\r");	
					}
            ...
            case VERIFY_MAXIMA:				
                ...
                {   
					if (mot_debug) {
						app_trace_log("exp_step: Max TO\r" );	
					}
			...
			case PEAK_STATS:
				...
				{
					if (mot_debug) {
						app_trace_log("exp_step: Too Long\r");	
					}
				...
				{
					if (mot_debug) {
						app_trace_log("exp_step: Too Soon\r");	
					}
				...
				{
					if (mot_debug) {
						app_trace_log("exp_step: Too Small\r");	
					}
				...
				if( ++Step.Cadence.ptr >= SAMP_SIZE ) Step.Cadence.ptr = 0;
				Step.Cadence.amplitudes[ Step.Cadence.ptr ] = Step.Maxima.mag;
			...
			case PEAK_TRACK:
				...
				else
				{
					...
					{   //Frequency too fast for bipedal motion
						if (mot_debug) {
							app_trace_log("exp_step: Cadence Too Fast\r");	
						}
						res = STEP_FILTER_CLR;
					}
					...
					{   //Amplitude is not high enough to match this frequency...
						...
						if (mot_debug) {
							app_trace_log("exp_step: Cadence Too Small\r");	
						}
						res = STEP_FILTER_CLR;
					}
					else
			...
			case UPDATE:
				...
				switch ( res )
				{
					case STEP_FILTER_CLR:
						if (mot_debug) app_trace_s_msg("exp_step: Clear\r");
					...
					case STEP_CNT_ADD:
						...
						Step.Abs_Count++;
						if (mot_debug) app_trace_log("exp_step: %01u\r", Step.Abs_Count);
