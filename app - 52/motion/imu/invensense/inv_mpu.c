...
#define log_i       printf
#define log_e       {NULL;}
#define get_ms(x)	*x = getSystemTimer();
...
status_code_t i2c_write(unsigned char addr, unsigned char regist, unsigned char len, unsigned char *data)
{
	...
	packet.i2c_id = addr;
	packet.address = regist;
	packet.length = len;
	packet.buffer = data;	      //pointer to return data
	stat = hal_twim_write(&packet);
	if( stat != STATUS_OK ){
		if (imu_debug) log_i("MPU Write Failed\r");
	}
	return stat;
}
...
status_code_t i2c_read(unsigned char addr, unsigned char regist, unsigned char len, unsigned char *reply)
{
	...
	if( stat != STATUS_OK ){
		if (imu_debug) log_i("MPU Read Failed\r");
	}
	return stat;
}
...
struct chip_cfg_s {
    ...
    unsigned char lpf;
	/* Matches accel_cfg2 & 0x07 register. */
...
enum lp_accel_rate_e {
    ...
    INV_LPA_0_3125HZ,	//0.24 Hz
    INV_LPA_0_625HZ,	//0.49 Hz
    INV_LPA_1_25HZ,		//0.98 Hz
    INV_LPA_2_5HZ,		//1.95 Hz
    INV_LPA_5HZ,		//3.91 Hz
    INV_LPA_10HZ,		//7.81 Hz
    INV_LPA_20HZ,		//15.63 Hz
    INV_LPA_40HZ,		//31.25 Hz
    INV_LPA_80HZ,		//62.50 Hz
    INV_LPA_160HZ,		//125 Hz
    INV_LPA_320HZ,		//250 Hz
    INV_LPA_640HZ		//500 Hz
#endif
};
...
{
	volatile status_code_t op_stat;
	
	// check of the part is there
	op_stat = i2c_read(st.hw->addr, st.reg->who_am_i, 1, reply);	//1 data bye is returned in reply
	if ( op_stat != STATUS_OK ) return (T_IMU_ERROR_CODE) op_stat;

	return ERROR_NONE;
}

/**
 *  @brief      Function with external scope to Enable/disable data ready interrupt.
 ...
 *  @return     0 if successful.
 */
int mpu_read_reg(unsigned char regist, unsigned char *data)
{
    if (regist == st.reg->fifo_r_w || regist == st.reg->mem_r_w)
        return -1;
    if (regist >= st.hw->num_reg)
        return -1;
    return i2c_read(st.hw->addr, regist, 1, data);
}
...
int mpu_init( void )
{
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (mpu_set_gyro_fsr(DEF_GYRO_FSR))
        return -1;
    if (mpu_set_accel_fsr(DEF_ACCEL_FSR))
        return -1;
    //if (mpu_set_lpf(DEF_LPF_CUTOFF))
        //return -1;
    if (mpu_set_sample_rate(DEF_ACCEL_SAMP_RATE))
        return -1;
...
#ifdef AK89xx_SECONDARY
    setup_compass();
    if (mpu_set_compass_sample_rate(DEF_MAG_SAMP_RATE))
        return -1;
#else
...
/**
 *  @brief      Read accelerometer values if device is in low power cycle mode.
 *  @return     true if low power mode in enabled.
 */
...
/**
 *  @brief      Set digital low pass filter.
 *  The following LPF settings are supported: 460, 184, 92, 41, 20, 10, 5.
 *  @param[in]  lpf Desired LPF setting.
 *  @return     0 if successful.
 */
int mpu_set_accel_lpf(unsigned short lpf)
{
    ...
    if (!(st.chip_cfg.sensors))
     ...
     data = INV_A_FILTER_184HZ;
		
	/* MPU6500 shares 4kB of memory between the DMP and the FIFO. Since the
    * first 3kB are needed by the DMP, we'll use the last 1kB for the FIFO.
    */
	data |= BIT_FIFO_SIZE_1024;
 ...
 *  @param[out] sens    Range of Gyro Reading.
 *  @return     0 if successful.
 */
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    st.chip_cfg.lp_accel_mode = 0;
    //delay_ms(50);		//you're high if you think I'm waiting here for 50 milliseconds...
    return 0;
}
...
#else
    gyro[0] = (long)(((int64_t)gyro[0]<<16) / test.gyro_sens / packet_count);
    gyro[1] = (long)(((int64_t)gyro[1]<<16) / test.gyro_sens / packet_count);
    gyro[2] = (long)(((int64_t)gyro[2]<<16) / test.gyro_sens / packet_count);
    accel[0] = (long)(((int64_t)accel[0]<<16) / test.accel_sens /
        packet_count);
    accel[1] = (long)(((int64_t)accel[1]<<16) / test.accel_sens /
        packet_count);
    accel[2] = (long)(((int64_t)accel[2]<<16) / test.accel_sens /
        packet_count);
    /* Don't remove gravity! */
    if (accel[2] > 0L)
        accel[2] -= 65536L;
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    gyro[0] = (long)(((long)gyro[0]<<16) / test.gyro_sens / s);
    gyro[1] = (long)(((long)gyro[1]<<16) / test.gyro_sens / s);
    gyro[2] = (long)(((long)gyro[2]<<16) / test.gyro_sens / s);
    accel[0] = (long)(((long)accel[0]<<16) / test.accel_sens / s);
    accel[1] = (long)(((long)accel[1]<<16) / test.accel_sens / s);
    accel[2] = (long)(((long)accel[2]<<16) / test.accel_sens / s);
    ...
    unsigned short lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = 0;				//user control reg (FIFO reset: BIT_FIFO_RST)
        data[1] = 0;				//power manage 1
        data[2] = BIT_STBY_XYZG;	//power manage 2
        ...
        data[0] = BIT_WOM_EN;	//|BIT_WOM_INTEL_MODE;
        if (i2c_write(st.hw->addr, st.reg->accel_intel, 1, data))
            goto lp_int_restore;
			
		// Start Low Power Accel only mode
		if ( mpu_lp_accel_mode(lpa_freq) )
			goto lp_int_restore;
...
lp_int_restore:
    ...
    data[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->accel_intel, 1, data)) {
        //goto lp_int_restore;	//this is a bloody infinite loop...
		return -1;
	}
#endif
...
/**
 *  @brief      Used to double check that the silence from the accelerometer 
 *				is due to a lack of motion and not some other problem that 
 *				reset the wake on motion interrupt.
 *
 *  @param[out] Contents of the accel intel register
 *  @return     0 if successful.
 */
