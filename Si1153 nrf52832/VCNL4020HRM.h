// .h for the Vishay Proximity/Ambient Light Sensor 4020

#include "nrf.h"

#define VCNL4020_ADDRESS               (0x13) //0x26 001 0011 shifted right 1 bit = 0x13
...
#define INTERRUPT_MASK_PROX_READY      (0x08)
    ...
    void SetCurrent (unsigned char CurrentValue);//
    void SetProximityRate (unsigned char ProximityRate);//
    void SetAmbiConfiguration (unsigned char AmbiConfiguration);
    void SetLowThreshold (unsigned int LowThreshold);
    void SetHighThreshold (unsigned int HighThreshold);
    void SetInterruptControl (unsigned char InterruptControl);//
    void SetInterruptStatus (unsigned char InterruptStatus);
    void SetModulatorTimingAdjustment (unsigned char ModulatorTimingAdjustment);
	void SetIRLEDAnode (uint8_t led_switch);//

    uint8_t ReadID (unsigned char *ID);
    uint8_t ReadCurrent (unsigned char *CurrentValue);
    uint8_t ReadCommandRegister (void);//
    uint16_t ReadProxiValue (void);//
    uint16_t ReadAmbiValue (unsigned int *AmbiValue);
    uint8_t ReadInterruptStatus (unsigned char *InterruptStatus);
    uint8_t ReadInterruptControl (unsigned char *InterruptControl);
    
    uint16_t ReadProxiOnDemand (unsigned int *ProxiValue);
    uint16_t ReadAmbiOnDemand (unsigned int *AmbiValue);
