                                                    ...
                                                    ble_gap_addr_t const * p_addr)
{
    ...
    for (index = 0; index < DEVICE_MANAGER_MAX_BONDS; index++)
    {
        ...
        if (m_peer_table[index].id_bitmap == UNASSIGNED)
        {
            ...
            DM_LOG("[DM]: Allocated Dev Inst 0x%02X\r\n", index);
...
static ret_code_t device_instance_find(ble_gap_addr_t const * p_addr, uint32_t * p_device_index, uint16_t ediv)
{
    ...
    for (index = 0; index < DEVICE_MANAGER_MAX_BONDS; index++)
    {
        ...
        {
            app_trace_log("[DM]: Found Bonded Dev at Inst 0x%02X\r\n", index);
	...
	// if bond not found erase old bond before bonding to the new device
	if(connected_and_bonded == 0)
	{
		//device_instance_free(0);
		app_trace_log("dm_bonded_device_delete The only one in the world.\r");
	}
