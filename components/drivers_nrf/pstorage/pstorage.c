 ...
 */
static void command_end_procedure_run( uint32_t res )
{    
    app_notify(res, &m_cmd_queue.cmd[m_cmd_queue.rp]);
...
static void state_entry_action_run(void)
{
    switch (m_state)
    {
        ...
        default:
            // No action needed.
			//maw: I disagree, move us into Idle State
			sm_state_change(STATE_IDLE);
            break;
    }
}
...
static void clear_post_processing_run(void)
{
    ...
    {
        command_end_procedure_run( NRF_SUCCESS );    
    }
    else
    {
...
{   
    ...
    {
        // Complete the operation with appropriate result code and transit to an error state.   
		// maw: transitioning to the Idle State to continue Operation
		command_end_procedure_run(NRF_ERROR_TIMEOUT);        
    }   
}
...
//Allow an application to check the status of p_store
uint32_t pstorage_state( void ) 
{
	return m_state;
}
