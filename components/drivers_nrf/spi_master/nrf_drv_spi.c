...
ret_code_t nrf_drv_spi_repair(nrf_drv_spi_t const * const p_instance,
                            ...
                            nrf_drv_spi_handler_t handler)
{
    ...
    {	//spi must have already been initialized, this is just a repeat if a failure occurs
        return NRF_ERROR_INVALID_STATE;
    }
	
	p_cb->transfer_in_progress = false;
	nrf_drv_spi_uninit( p_instance );
	
	res = nrf_drv_spi_init( p_instance, p_config, handler);
