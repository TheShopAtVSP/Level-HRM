...
static void bootloader_settings_save(bootloader_settings_t * p_settings)
{
	...
	//store latest version so application program's use
	p_settings->bl_rev = bl_version_get();	
...
void bootloader_dfu_update_process(dfu_update_status_t update_status)
{
    ...
    else if (update_status.status_code == DFU_TIMEOUT)
    {
        ...
        APP_ERROR_CHECK(err_code);
#ifdef APP_TRACE_ON
		app_trace_log(DEBUG_MED, "\n\rDFU_TIMEOUT in bootloader_dfu_update_process\r");
#endif
        m_update_status = BOOTLOADER_TIMEOUT;
    }
...
void bootloader_settings_get(bootloader_settings_t * const p_settings)
{
	...
	p_settings->bl_rev         = p_bootloader_settings->bl_rev;
}
