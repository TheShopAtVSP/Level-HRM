...
uint32_t log_rtt_remote_console(int terminal_index, char * p_buffer, uint32_t len)
{
	...
	if( last_index != _SEGGER_RTT.aUp[terminal_index].RdOff ) {
		if( last_index > 0 ) {
			//there has been a read of the buffer outside this routine!
			last_index = _SEGGER_RTT.aUp[terminal_index].RdOff;
		}
	}
	
	read_len = SEGGER_RTT_UpBuffer_Retrieve( terminal_index, (char *) p_buffer, len );
