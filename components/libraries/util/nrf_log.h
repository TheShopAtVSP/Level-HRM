...
// Send RTT data out to a remote debug terminal
uint32_t log_rtt_remote_console(int terminal_index, char * p_buffer, uint32_t len);
...
#define NRF_LOG_REMOTE(p_char, val)		log_rtt_remote_console(LOG_TERMINAL_NORMAL, p_char, val)   
...
#define NRF_LOG_REMOTE(p_char, val)		0  
...
/**@brief Macro for reading data from the RTT Buffer to send to a remote console.
 *
 * @param[out]   p_char  Pointer where to store the characters.
 *
 * @retval      length of bytes copied to the buffer
 * @retval      NRF_ERROR_INVALID_DATA If no character could be read.
 */
uint32_t NRF_LOG_REMOTE(char* p_char, uint len);
