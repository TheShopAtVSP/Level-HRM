...
uint32_t systick_ms = 0; /* Variable to store millisecond ticks */
...
int main(void)
{
	...
	if (SysTick_Config(SystemCoreClock / 100) != NRF_SUCCESS) 
	{
		NVIC_SystemReset();
	}
	
	// Init i2c interface to setup the Battery Charger
	hal_twim_init( false );
	if( bq25120_init() != NRF_SUCCESS ) 
	{
		NVIC_SystemReset();
	}
	hal_twim_uninit();

	// Set up Serial Port for Bluetooth Direct Test Mode HCI:
    uart_init();
	...
	//Set Test Mode to an Initial state:
	//dtm_error_code = dtm_cmd(LE_TRANSMITTER_TEST, 0, 0, DTM_PKT_VENDORSPECIFIC);	//Continuos Carrier Low  Channel: 2.402 GHz
	//dtm_error_code = dtm_cmd(LE_TRANSMITTER_TEST, 19, 0, DTM_PKT_VENDORSPECIFIC);	//Continuos Carrier Mid  Channel: 2.440 GHz
	//dtm_error_code = dtm_cmd(LE_TRANSMITTER_TEST, 39, 0, DTM_PKT_VENDORSPECIFIC);	//Continuos Carrier High Channel: 2.480 GHz
	dtm_error_code = dtm_cmd(LE_TRANSMITTER_TEST, 0, 10, DTM_PKT_PRBS9);			//BLE Spec defined bit pattern
	if( dtm_error_code != DTM_SUCCESS )
	{
		NVIC_SystemReset();
	}
	
	// setup output pin to turn on LED as Proof of Life
